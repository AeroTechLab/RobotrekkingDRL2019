...
rover-3-wheels.xml qpos and qvel
...
qpos[11]= front center wheel steer rotation angle
qpos[12]= front center wheel rolling rotation angle
...
qpos[6] = rear left wheel steer angular velocity
qpos[7] = rear left wheel rolling angular velocity
qpos[8] = rear right wheel steer angular velocity
qpos[9]= rear right wheel rolling angular velocity
qpos[10]= front center wheel steer angular velocity
qpos[11]= front center wheel rolling angular velocity
...
class RoverRobotrek3WEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def __init__(self):
        mujoco_env.MujocoEnv.__init__(self, 'main-trekking-challenge-3wheels.xml', 4)  # __init__(self, model_path, frame_skip):
    ...
    def _get_obs(self):
        ...
        steer_wheel_angle = self.sim.data.qpos[11]
        steer_wheel_angspeed = self.sim.data.qvel[10]
        ...
        return gps_exact, np.concatenate([gps_sensor.flat, orientation_rover.flat, [steer_wheel_angle], speed_sensor.flat, [rover_ang_speed], [steer_wheel_angspeed], goal.flat, coordinates_goal.flat])
